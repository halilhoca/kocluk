...
const StudentExams: React.FC = () => {
  ...
  const prepareChartData = () => {
    ...
    // Filtrelenmiş sınavları al
    const filteredExams = exams.filter(exam => selectedType === 'ALL' || exam.exam_type === selectedType);
    
    if (filteredExams.length === 0) return null;

    // Benzersiz deneme başlıklarını bul (calculateCustomStatistics ile aynı mantık)
    const examsByName: { [key: string]: StudentExam[] } = {};
    ...
    const examData: { date: string; totalNet: number; examName: string }[] = [];
    ...
    Object.entries(examsByName).forEach(([examName, examList]) => {
      ...
      // En son tarihli sınavı al
      const latestExam = examList.sort((a, b) => new Date(b.exam_date).getTime() - new Date(a.exam_date).getTime())[0];
      examData.push({ date: latestExam.exam_date, totalNet, examName });
    });

    // Tarihe göre sırala
    examData.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());

    const labels = examData.map((exam, index) => {
      const date = new Date(exam.date);
      return exam.examName || `${index + 1}. Deneme (${date.getDate()}/${date.getMonth() + 1})`;
    });

    const netScores = examData.map(exam => exam.totalNet);
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `${selectedType === 'ALL' ? 'Tüm Denemeler' : selectedType} - Net Puan Gelişimi`,
        font: {
          size: 18,
          weight: 'bold' as const,
        },
